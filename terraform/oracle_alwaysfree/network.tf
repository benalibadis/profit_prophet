...
resource "oci_core_security_list" "alwaysfree_security_list" {
  ...
  dynamic "ingress_security_rules" {
    ...
    content {
      protocol = var.ingress_security_rules.value.protocol
      source   = var.ingress_security_rules.value.source
      tcp_options {
        min = var.ingress_security_rules.value.min_port
        max = var.ingress_security_rules.value.max_port
      }
    }
  }
